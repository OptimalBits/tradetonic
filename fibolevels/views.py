...
def embed_svg3( svg ):
    return '<object data=' + "'data:image/svg+xml," + str(svg) + "'" + 'type="image/svg+xml" width="400" height="250"></object>'
...
def renderFiboChart( quote_id, resolution = 1 ):
    ...
    if resolution >= 1 and resolution < 100:
        ...
        pattern = find_last_pattern2( q ) 
        
        longName = find_name( quote_id, shares + indexes ).longName
        a = QuoteAnalysis(longName, 16, q)
        rl = a.get_retracements( *a.last_pattern )
        
        return FibonacciChart( "Fibonacci Levels", a.name, ticks, rl, resolution, pattern )
...
def update_params( request, quote_id, resolution = 1, tabs = None):
    ...
    if resolution >= 1 and resolution < 100:
        ...
        #svg = retracementPatternChart("Retracement Pattern", None, [p1,p2,p3,p4] )
        
        #return HttpResponse( embed_svg3( svg ) )
